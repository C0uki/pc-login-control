// =====================================================
// SHA-256（純JavaScript・UTF-8対応）
//   管理コンソールでパスワードをハッシュ化するために使用。
//   crypto.subtle に依存しないため、http / file:// を含む
//   どの環境でも動作します（サーバー/GAS/RN と同一の出力）。
// =====================================================
(function (global) {
  'use strict';
  ...
  function sha256Bytes(bytes) {
    ...
    for (i = 0; i < out.length; i++) {
      hex += ('00000000' + (out[i] >>> 0).toString(16)).slice(-8);
    }
    return hex;
  }

  /** 文字列（UTF-8）の SHA-256 を小文字HEXで返す */
  function sha256(str) {
