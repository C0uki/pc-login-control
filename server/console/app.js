...
async function api(payload) {
    ...
    try {
        ...
        throw new Error('レスポンス解析に失敗: ' + text.slice(0, 160));
    }
}
...
function authCall(action, extra) {
    ...
    return api(Object.assign({ action, orgId: session.orgId, userId: 'MASTER', passwordHash: session.passwordHash }, extra || {}));
}
function esc(s) {
...
async function refreshHealth() {
    ...
    try {
        ...
        if (r.signupCodeRequired)
            $('signupCodeRow').classList.remove('hidden');
        const notes = [];
        if (!r.envConfigured)
            notes.push('環境変数 SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY を設定してください。');
        if (r.envConfigured && !r.tablesReady)
            notes.push('DBテーブルが未作成です（スキーマSQLを実行）。');
        if (r.dbError)
            notes.push('DB: ' + r.dbError);
...
async function createOrg() {
    ...
    if (!name) {
        showMsg(err, '組織名を入力してください。', 'error');
        return;
    }
    if (!pw) {
        showMsg(err, '管理者パスワードを入力してください。', 'error');
        return;
    }
    inp('createOrgBtn').disabled = true;
    try {
        const payload = { action: 'createOrg', orgName: name, masterPasswordHash: sha256(pw) };
        if (code)
        ...
        inp('createOrgBtn').disabled = false;
    }
}
// ---------- 組織にログイン ----------
async function doLogin() {
    ...
    err.classList.add('hidden');
    if (!orgId) {
        showMsg(err, '組織IDを入力してください。', 'error');
        return;
    }
    if (!pw) {
        showMsg(err, '管理者パスワードを入力してください。', 'error');
...
async function loadUsers() {
    ...
    try {
        const r = await authCall('listUsers');
        if (!r.success) {
            ul.innerHTML = '<li class="empty">' + esc(r.message) + '</li>';
            return;
        }
        const users = r.users || [];
        if (!users.length) {
            ul.innerHTML = '<li class="empty">ユーザーがいません</li>';
            return;
        }
        ul.innerHTML = '';
        users.forEach((u) => {
            ...
            if (btn)
                btn.addEventListener('click', () => deleteUser(u.userId));
            ul.appendChild(li);
        });
    }
    catch (e) {
        ul.innerHTML = '<li class="empty">' + esc(errMsg(e)) + '</li>';
    }
}
async function addUser() {
    ...
    const msg = $('userMsg');
    if (!id || !pw) {
        showMsg(msg, 'ユーザーIDと初期パスワードは必須です。', 'error');
        return;
    }
    try {
        const r = await authCall('register', { newUserId: id, newUserName: name, newPasswordHash: sha256(pw) });
        if (r.success) {
            showMsg(msg, 'ユーザー「' + id + '」を登録しました。', 'ok');
            inp('newUserId').value = '';
            inp('newUserName').value = '';
            inp('newUserPw').value = '';
            loadUsers();
        }
        else {
...
async function deleteUser(userId) {
    ...
    const msg = $('userMsg');
    try {
        const r = await authCall('deleteUser', { targetUserId: userId });
...
async function loadLogs() {
    ...
    ul.innerHTML = '<li class="empty">読み込み中…</li>';
    try {
        const r = await authCall('getLogs', { limit: 100 });
        const logs = (r.success && r.logs) ? r.logs : [];
        if (!logs.length) {
            ul.innerHTML = '<li class="empty">ログはありません</li>';
            return;
        }
        ul.innerHTML = '';
        logs.forEach((log) => {
...
function actionLabel(a) {
    if (a.indexOf('login') === 0)
        return a.indexOf('mobile') >= 0 ? 'ログイン(スマホ承認)' : 'ログイン';
    if (a === 'logout')
        return 'ログアウト';
    return a;
}
...
async function loadApprovals() {
    const ul = $('approvalList');
    ul.innerHTML = '<li class="empty">読み込み中…</li>';
    try {
        const r = await authCall('listRequests');
        const reqs = (r.success && r.requests) ? r.requests : [];
        if (!reqs.length) {
            ul.innerHTML = '<li class="empty">保留中の承認はありません</li>';
            return;
        }
        ul.innerHTML = '';
        reqs.forEach((req) => {
            ...
            if (ap)
                ap.addEventListener('click', () => respond(req.requestId, 'approve'));
            if (dn)
                dn.addEventListener('click', () => respond(req.requestId, 'deny'));
            ul.appendChild(li);
        });
    }
    catch (e) {
...
async function respond(requestId, decision) {
    try {
        await authCall('respondRequest', { requestId, decision });
    }
    catch (e) { /* 再読込に任せる */ }
    loadApprovals();
}
...
function switchTab(name) {
    ['users', 'logs', 'approvals'].forEach((t) => {
...
function init() {
    ...
    $('saveApiUrl').addEventListener('click', () => {
        ...
        if (v)
            localStorage.setItem('pclc.apiBase', v);
        else
            localStorage.removeItem('pclc.apiBase');
    ...
    inp('loginMasterPw').addEventListener('keydown', (e) => { if (e.key === 'Enter')
        doLogin(); });
